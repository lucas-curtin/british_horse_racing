"""Hierarchical Bayesian model for full-train fitting and fixture predictions."""
...
TrainArtifacts = tuple[az.InferenceData, StandardScaler, dict[str, np.ndarray], dict[str, pd.Index]]


def train_on_all() -> TrainArtifacts:
    """Fit the hierarchical model on the full merged historical dataset."""
...
def predict_and_summarize(
    ...
    """Generate fixture-level probability summaries from a fitted model."""
    ...
    # Scale features
    x_future = fdf[RAW_FEATURES].fillna(0.0)
    ...
    mean_betas = post["betas"].mean(dim=("chain", "draw")).to_numpy()
    mean_gi = post["global_intercept"].mean(dim=("chain", "draw")).to_numpy()
    re_means = {
        key: post[f"offset_{key}"].mean(dim=("chain", "draw")).to_numpy()
        for key in RE_KEYS
    }

    # Calculate win probabilities
    logits = mean_gi + x_future_scaled.dot(mean_betas)
    for key in RE_KEYS:
    ...
    with BET_FILE.open("w", encoding="utf-8") as bf:
        for (f, r), grp in fdf.groupby(["fixture_index", "race_index"]):
            ...
            bf.writelines(f"  {row.Horse}: {row.Prob:.2%}\n" for _, row in preds.iterrows())
            top = preds.iloc[0]
            bf.write(f"Top pick: {top.Horse} ({top.Prob:.2%})\n\n")
    logger.info("Bet summary written to {}", BET_FILE)
...
if __name__ == "__main__":
    ...
    del idx
    predict_and_summarize(trace, scaler, labels)
